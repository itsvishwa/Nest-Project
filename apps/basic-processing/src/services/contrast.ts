...
export class ContrastService {
  private applyContrast(
    ...
    // Calculate the contrast correction factor
    const factor = (259 * (contrast + 255)) / (255 * (259 - contrast));

    for (let i = 0; i < imageData.length; i += channels) {
      for (let c = 0; c < channels; c++) {
        const originalValue = imageData[i + c];
        const newValue = factor * (originalValue - 128) + 128;
        result[i + c] = Math.max(0, Math.min(255, Math.round(newValue))); // Truncate to valid range
      }
    }
